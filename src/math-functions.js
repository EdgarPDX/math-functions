...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    const sumAB = sum(a, b,);
    const sumAB0 = sum(sumAB [0], c);
    const multiAB = multiply(a, b);
    const multiAB0 = multiply(multiAB [0], c);

    const multiResult = multiply(a, b, multiply(c));
    return [sumAB0 [0], multiAB0 [0], `${a} and ${b} and ${c} sum to ${sumAB0[0]}.`, `The product of ${a} and ${b} and ${c} is ${multiAB0[0]}.`];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    ...
    const sumAB = sum(sumArr[0], sumArr[1]);
    const sumAB0 = sum(sumAB[0], sumArr[2]);
    
    return [sumAB0[0], `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${sumAB0[0]} is their sum.`];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const multAB = multiply(multArr[0], multArr[1]);
    const multAB0 = multiply(multAB[0], multArr[2]);
    
    return [multAB0[0], `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${multAB0[0]}.`];
}
